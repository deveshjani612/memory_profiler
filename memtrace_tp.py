...
try:
    ...
    logf.close()
    print("\nSaved: memtrace_log.csv")#!/usr/bin/env python3
from bcc import BPF
import argparse, time, csv, sys, os, struct
...
def virt_to_pfn(pid, vaddr):
    """
    Decode /proc/[pid]/pagemap entry for a virtual address.
    Extract PFN bits (0–54) exactly as per kernel definition.
    Returns (present, swapped, soft_dirty, file_page, pfn or None)
    """
    try:
        ...
        # ---- Decode bits per /Documentation/admin-guide/mm/pagemap.rst ----
        present    = (entry >> 63) & 1   # Bit 63: page present in RAM
        swapped    = (entry >> 62) & 1   # Bit 62: page swapped out
        soft_dirty = (entry >> 55) & 1   # Bit 55: page written since last clear
        file_page  = (entry >> 54) & 1   # Bit 54: page mapped to file/shared
        pfn_bits   = entry & ((1 << 55) - 1)  # Bits 0–54: PFN (if present)

        if not present or swapped:
            return present, swapped, soft_dirty, file_page, None

        # PFN valid only if present=1 and swapped=0
        return present, swapped, soft_dirty, file_page, pfn_bits
...
def on_event(cpu, data, size):
    ...
    # Compute PFN for faults (optional for others)
    if etype == "fault":
        present, swapped, soft_dirty, file_page, pfn = virt_to_pfn(target_pid, e.addr)
        pfn_str = hex(pfn) if pfn is not None else "N/A"
        print(f"[{time.strftime('%H:%M:%S')}] pid={e.pid:<6} {etype:<6} addr=0x{e.addr:x} len={e.len} "
              f"pfn={pfn_str} (present={present}, swapped={swapped}, soft_dirty={soft_dirty}, file={file_page})")
    else:
        pfn_str = "N/A"
        print(f"[{time.strftime('%H:%M:%S')}] pid={e.pid:<6} {etype:<6} addr=0x{e.addr:x} len={e.len}")
